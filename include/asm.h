...
#define bound() __asm__ volatile("bound"::)
...
#define popb(value) __asm__ volatile("popb %0":"=g"(value):)
#define popw(value) __asm__ volatile("popw %0":"=g"(value):)
#define popl(value) __asm__ volatile("popl %0":"=g"(value):)
...
#define get_ds() ({                                             \
    unsigned short _v;                                          \
    __asm__ volatile("movw %%ds, %%ax":"=a"(_v):);              \
    _v;                                                         \
})

#define get_es() ({                                             \
    unsigned short _v;                                          \
    __asm__ volatile("movw %%es, %%ax":"=a"(_v):);              \
    _v;                                                         \
})

#define get_fs() ({                                             \
    unsigned short _v;                                          \
    __asm__ volatile("movw %%fs, %%ax":"=a"(_v):);              \
    _v;                                                         \
})

#define get_gs() ({                                             \
    unsigned short _v;                                          \
    __asm__ volatile("movw %%gs, %%ax":"=a"(_v):);              \
    _v;                                                         \
})

#define get_ss() ({                                             \
    unsigned short _v;                                          \
    __asm__ volatile("movw %%ss, %%ax":"=a"(_v):);              \
    _v;                                                         \
})
